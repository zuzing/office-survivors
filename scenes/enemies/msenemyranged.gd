...
func _ready() -> void:
	...
	if not resource.projectile_scene:
		var default_path = "res://scenes/entities/projectile/ms_projectile.tscn"
...
func _move_toward_player(_delta: float) -> void:
	var ranged_speed = resource.speed
...
func _maintain_distance(delta: float) -> void:
	var distance = global_position.distance_to(player.global_position)
	var target_distance = preferred_distance
...
func _shoot_projectile(direction: Vector2) -> void:
	...
	elif projectile.has_method("set_velocity_data"):
		resource.projectile.set_velocity_data(direction * resource.projectile_speed)
